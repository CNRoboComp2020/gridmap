...
vector<pairii> getCellLists(Eigen::Vector2d pt1, Eigen::Vector2d pt2, double res, Eigen::Vector2d origin)
{
    ...
    else
    {
        ...
        {
            if (intersect_list_x[ptrx].first < intersect_list_y[ptry].first)
            {
...
VisibleGridNode::VisibleGridNode()
{
    ...
    this->publishloop_timer_ = n_.createTimer(ros::Duration(0.01), &VisibleGridNode::publishloopCallback, this);
}
...
{    
    ...
    while (yaw <= yaw_max)
    {
        ...
        for (auto iter = cell_list.begin(); iter != cell_list.end(); iter++)
        {
            ...
            //cout << "(" << iter->first << "  " << iter->second << ") success\n";
        }
        yaw += yaw_step;
    }
    ...
    std::cout << "time consumed "<< deltaT <<std::endl;
}
...
{
/*
    Eigen::Vector2d start(600, 500), end(400, 400), origin(0, 0);
